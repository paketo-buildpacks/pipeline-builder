...
import (
	...
	"github.com/paketo-buildpacks/pipeline-builder/v2/octo/builder"
)
...
Bumps %[1]s from ${{ steps.build-image.outputs.old-version }} to ${{ steps.build-image.outputs.new-version }}.`, image),
							...
							"labels":        "${{ steps.build-image.outputs.version-label }}, type:dependency-upgrade",
						},
					},
				},
			},
...
Bumps lifecycle from ${{ steps.lifecycle.outputs.old-version }} to ${{ steps.lifecycle.outputs.new-version }}.`,
							...
							"labels":        "${{ steps.lifecycle.outputs.version-label }}, type:dependency-upgrade",
						},
					},
				},
			},
