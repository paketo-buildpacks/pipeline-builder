...
func (a AdoptOpenJDK) Check(request CheckRequest) (CheckResult, error) {
	versions, err := a.Versions(request.Source)
	if err != nil {
...
func (a AdoptOpenJDK) In(request InRequest, destination string) (InResult, error) {
	...
	sha256, err := DownloadArtifact(uri, request.Version, destination)
	if err != nil {
		return InResult{}, fmt.Errorf("unable to download %s: %w", uri, err)
	}
